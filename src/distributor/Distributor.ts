...
class Distributor extends EventEmitter.EventEmitter {
  ...
  readRouter(id: ObjectId): Promise<Router<ObjectId> | null> {
    return this._db.collection<Router<ObjectId>>(Collections.ROUTERS).findOne({
      _id: id,
    });
  }
  ...
  readLocalAudioTrack(id: ObjectId): Promise<LocalAudioTrack<ObjectId> | null> {
    return this._db
      .collection<LocalAudioTrack<ObjectId>>(Collections.LOCAL_AUDIO_TRACKS)
      .findOne({
        _id: id,
  ...
  readLocalVideoTrack(id: ObjectId): Promise<LocalVideoTrack<ObjectId> | null> {
    return this._db
      .collection<LocalVideoTrack<ObjectId>>(Collections.LOCAL_VIDEO_TRACKS)
      .findOne({
        _id: id,
  ...
  updateUserWithPermissions(
    ...
    return this._db
      ...
      .then((result) => {
        ...
        throw new Error(`Could not find and update user with permission ${id}: ${result.lastErrorObject}`);
      });
  }
    ...
    return this._db
      ...
      .then((result) => {
        if (result.value) {
          ...
          return Promise.all([
            this._db
              .collection<LocalAudioTrack<ObjectId>>(
                Collections.LOCAL_AUDIO_TRACKS
              )
              .find(
                {
                  deviceId: id,
            ...
            this._db
              .collection<LocalVideoTrack<ObjectId>>(
                Collections.LOCAL_VIDEO_TRACKS
              )
              .find(
                {
                  deviceId: id,
  ...
  async joinStage(
    ...
    await this._db
      ...
      .then((devices) =>
        devices.map((device) =>
          this.upsertCustomStageMemberVolume(
            userId,
            stageMember._id,
            device._id,
            {
        ...
        await this._db
          ...
          .then((producers) =>
            producers.map((producer) =>
              this.deleteRemoteAudioTrack(producer._id)
            )
          );
        await this._db
          .collection<RemoteVideoTrack<ObjectId>>(
  ...
  leaveStageForGood(userId: ObjectId, stageId: ObjectId): Promise<any> {
    return this.readUser(userId).then(async (user) => {
      if (user) {
  ...
  private async getWholeStage(
    ...
    const customRemoteAudioTrackVolumes: CustomRemoteAudioTrackVolume<ObjectId>[] = await this._db
      ...
      .find({
        userId,
        ObjectId: {
          $in: remoteAudioTracks.map(
            (remoteAudioTrack) => remoteAudioTrack._id
          ),
        },
      })
      .toArray();
  ...
  deleteStage(id: ObjectId): Promise<any> {
    return this._db
      ...
      .then(stage => {
        if (stage) {
          // Remove groups first
          return this._db.collection<Group<ObjectId>>(Collections.GROUPS)
            .find({stageId: id}, {projection: {_id: 1, videoRouter: 1, audioRouter: 1}})
            .toArray()
            .then(groups => {
              // Delete groups
            ...
            .then(() => this._db.collection<Stage<ObjectId>>(Collections.STAGES).deleteOne({_id: id}));
        }
  ...
  /*
  deleteTrackPreset(userId: ObjectId, id: TrackPresetId): Promise<any> {
    return this._db
      ...
      .then((result) => {
        if (result.value) {
          this.emit(ServerEvents.TRACK_PRESET_REMOVED, id);
          this.sendToUser(
            result.value.userId,
            ServerEvents.TRACK_PRESET_REMOVED,
            id
          );
          // Delete all custom stage members and stage member tracks
          return this._db
    ...
    return this._db
      ...
      .then((result) => {
        if (result.value) {
          ...
          this.emit(ServerEvents.TRACK_PRESET_CHANGED, payload);
          return this.sendToUser(
            result.value.userId,
            ServerEvents.TRACK_PRESET_CHANGED,
            payload
          );
          // TODO: Remove tracks?
        }
  ...
  readCustomGroupPosition(
    id: ObjectId
  ): Promise<CustomGroupPosition<ObjectId>> {
    return this._db
      .collection<CustomGroupPosition<ObjectId>>(
        Collections.CUSTOM_GROUP_POSITIONS
      )
        ...
        if (result.ok) {
          return this.readStageMember(stageMemberId)
            .then(
              (
                stageMember
              ): Omit<CustomStageMemberPosition<ObjectId>, "_id"> => ({
                x: stageMember.x,
                y: stageMember.y,
                z: stageMember.z,
                rX: stageMember.rX,
        ...
        if (result.ok) {
          return this.readStageMember(stageMemberId)
            .then(
              (
                stageMember
              ): Omit<CustomStageMemberVolume<ObjectId>, "_id"> => ({
  ...
  deleteCustomRemoteAudioTrackPosition(id: ObjectId): Promise<void> {
    return this._db
      ...
      .then((result) => {
        ...
        throw new Error(
          `Could not find and delete remote audio track position ${id}`
        );
      });
  }
  ...
  deleteCustomRemoteAudioTrackVolume(id: ObjectId): Promise<void> {
    return this._db
      ...
      .then((result) => {
        ...
        throw new Error(
          `Could not find and delete custom remote audio track volume ${id}`
        );
      });
  }
  ...
  public async sendStageDataToDevice(
    ...
    if (user.stageMemberId) {
      ...
      if (stageMember) {
        const wholeStage = await this.getWholeStage(
          user._id,
          user.stageId,
          true
        );
      ...
      .then((presets) =>
        presets.map((trackPreset) =>
          MongoRealtimeDatabase.sendToDevice(
            socket,
            ServerEvents.TRACK_PRESET_ADDED,
            trackPreset
          )
        )
      ); */
