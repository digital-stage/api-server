...
class Distributor extends EventEmitter.EventEmitter {
        ...
        if (result.value) {
          ...
          if (update.active !== undefined) {
            ...
            await this._db
              ...
              .then((stageDevices) =>
                stageDevices.map((stageDevice) => {
                    const device = this._db.collection<Device<ObjectId>>(Collections.DEVICES)
                      .findOne({_id: stageDevice.userId, online: true})
                      .then(device => {
                        if (device) {
                          this.updateStageDevice(stageDevice._id, {
  ...
  updateStageDevice = (
    id: ObjectId,
    update: Partial<Omit<StageDevice<ObjectId>, '_id' | 'stageId' | 'userId' | 'stageMemberId' | 'order'>>
  ): Promise<void> =>
    this._db
      .collection<StageDevice<ObjectId>>(Collections.STAGE_DEVICES)
      .findOneAndUpdate(
